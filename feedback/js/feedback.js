...
class ItcSubmitForm {
  ...
  _setStateValidaion(input, state, message) {
    ...
    if (input.closest('.form-group').querySelector('.invalid-feedback')) { input.closest('.form-group').querySelector('.invalid-feedback').textContent = ''; }
	//console.log(input);
  ...
  _checkValidity() {
    let valid = true;
    // input, textarea
    this._elForm.querySelectorAll('input[type="text"], input[type="email"], input[type="checkbox"], input[type="file"], textarea').forEach(el => {
      if (el.type === 'file') {
        return;
      }
      if (el.checkValidity()) {
