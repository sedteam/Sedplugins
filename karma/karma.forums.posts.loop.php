...
if (!isset($karma[$row['fp_posterid']]['karma']) || !$karma[$row['fp_posterid']]['karma']) {
	...
	$value = ($tmp['karma'] >= 100) ? 100 : $tmp['karma']; //правка цвета для очень большой кармы
	$value = ($tmp['karma'] <= -100) ? 100 : $value; //правка цвета для очень большой кармы

	$hex = dechex(floor(256 - abs($value) * 2.56)); //просчет цвета
	$hex = (strlen($hex) == 1) ? "0" . $hex : $hex; // правка цвета
	$hex = ($hex == 100) ? "ff" : $hex; // для нулевой кармы

	$color = ($value > 0) ? $hex . "ff" . $hex : "ff" . $hex . $hex;
	$color = ($value == 0) ? "ffffff" : $color;
...
$t->assign(array(
	"FORUMS_POSTS_ROW_KARMA_ADD" => "<a href=\"javascript:sedjs.popup('karma&act=change&a=add&fp=" . $row['fp_id'] . "','700','500', " . $modal . ", 'Karma')\"><nobr>[+]</nobr></a>",
	"FORUMS_POSTS_ROW_KARMA_DEL" => "<a href=\"javascript:sedjs.popup('karma&act=change&a=del&fp=" . $row['fp_id'] . "','700','500', " . $modal . ",'Karma')\"><nobr>[-]</nobr></a>",
	"FORUMS_POSTS_ROW_KARMA" => "<a href=\"javascript:sedjs.popup('karma&act=show&fp=" . $row['fp_posterid'] . "','700','500', " . $modal . ", 'Karma')\" style=\"background-color:#" . $karma[$row['fp_posterid']]['color'] . "\"><nobr>Karma " . $karma[$row['fp_posterid']]['karma'] . "</nobr></a>"
));
